...
n_hidden_1 = 1  # HiddenLayer 1 dimensions
...
# Create MLP Model
def create_Perceptron(x, weights, biases):
    perceptron = tf.add(tf.matmul(x, weights['perceptron']), biases['bias'])
    perceptron = tf.nn.sigmoid(perceptron)
    return perceptron


# Store layers weight & bias
weights = {
    'perceptron': tf.Variable(tf.random_normal([n_input, n_hidden_1])),
}
biases = {
...
# Construct model
output_layer = create_Perceptron(X, weights, biases)
